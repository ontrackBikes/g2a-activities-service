...
Availability/pricing lookups for the customer-facing app (`getProductsListForApp`, `getProductDetailsForApp`, etc. in `controllers/product.controller.js`) go through `services/availableVendor.service.js` and `services/nextAvailableSlot.service.js`, and the booking-quote logic lives under `services/availability/` (`singleDate.service.js`, `dateRange.service.js`, `airportTransfer.service.js`/`cabService.service.js` (both thin wrappers over `transfer.service.js`), `buildBookingQuote.js`, `availableDates.service.js` — dispatched based on `Product.booking_mode`: `single_date` / `date_range` / `open`). Each handler independently populates `availability.service_hours` on the quote it builds (`buildBookingQuote.js` just passes it through unchanged) as an array of `{ start_time, end_time }` — one entry per open slot for `SLOT`-priced products, one entry from the default schedule slot otherwise, `null` if neither has both times set. There's no shared helper for this (each file has its own local `getServiceHours`, matching the existing duplication of `getEffectiveMaxBookable`/`getSlotType` across these files) — update all three handlers together when changing this field.

Checkout payloads are validated per "section": `constants/bookingSections.js` enumerates every possible section (`participants`, `kyc_per_passanger`, `rental_details`, `medical_declaration`, ...); each product's `BookingTemplate.booking_page_schema.sections` array turns individual sections on/off (and required/optional) for that product; `schemas/bookingPayload.schema.js#validateBookingPayload` only Joi-validates the sections a given product has enabled. A new section needs registering in both `BOOKING_SECTIONS` and `SECTION_SCHEMAS` to be picked up. `controllers/order.controller.js#createOrderService` layers extra cross-checks beyond Joi for sections that need them beyond per-item shape validation — e.g. `participants` and `kyc_per_passanger` array lengths must each independently equal `estimate.booking_data.guests` (they are *not* assumed to correspond to each other by index — treat every section as its own independent list unless explicitly told otherwise), gated behind the same `section.enabled` check (`hasParticipantsSection`/`hasKycSection` pattern). Follow that `hasXSection` + `throw { status, message, errors }` pattern for new cross-field section checks rather than trying to express them in Joi.
...
Four job domains: media processing (image variants via `sharp`), media cleanup (orphaned upload cleanup, plus expiring `documents` rows — see below), vendor schedule maintenance (inventory generation described above), payment settlement (`services/paymentSettlement.service.js` — locks a `Payment` row with `settlement_status`, confirms/settles bookings, sends Postmark emails via `emailTemplates` constants).

### Documents (generic uploads / KYC)

`Document` (table `documents`) is a separate upload model from `MediaLibrary` — one stored file per row, no image-variant processing/queue, loosely tied to whatever it belongs to via `entity_type`/`entity_id` (no FK, same polymorphic-by-convention pattern `MediaLibrary` uses with just `entity_type`), plus its own `status` (`active`/`deleted`) and a nullable `expires_at` (`null` = kept forever). `services/document.service.js` owns upload/list/get/delete. `services/mediaCleanup.service.js#runMediaCleanup` — the same function the existing `media-cleanup` cron/worker already calls — also calls `cleanupExpiredDocuments()`; unlike media's temp/bin cleanup (which hard-deletes rows), expired documents keep their row with `status: "deleted"` as an audit trail while the file itself is removed from disk. `POST /api/v1/booking-estimates/:estimate_id/upload-kyc` is the one concrete caller today: it tags uploads with `entity_type: "estimate"` and `entity_id` set to `BookingEstimate.id` (the internal BIGINT PK — not the public UUID `estimate_id` from the URL), and always sets `expires_at` to +1 month server-side (never client-supplied).
