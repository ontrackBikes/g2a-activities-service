...
const getinfoBikeRentals = (req, res) => {
  try {
    const locations = bikeRentals.getLocations();
    res.json({
      success: true,
      product: bikeRentals.productInfo(),
      locations: locations,
    });
  } catch (error) {
...
const checkAvailabilityBikeRentals = (req, res) => {
  try {
    ...
    // Call service
    const result = bikeRentals.checkAvailabilityPreflight({
      locationName,
      startDate,
      endDate,
      quantity,
...
const getPickupDropPointsByLocation = (req, res) => {
  try {
    ...
    const result = bikeRentals.getPickupDropPoints(locationName);
...
const getBikeRentalLocationByName = (req, res) => {
  try {
    ...
    const location = bikeRentals.getLocationByName(locationName);
